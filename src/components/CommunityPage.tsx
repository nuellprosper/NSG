            ...
            <div className="bg-gradient-to-br from-[#1E1B2E] to-[#120F1F] p-6 rounded-[2.5rem] shadow-2xl space-y-6 text-left border border-white/5">
                  ...
                  {quests.map((qst, idx) => {
                    ...
                    const handleQuestAction = () => {
                      if (canClaim) {
                        ...
                        setUserNotification("ðŸŒˆ Daily quest already claimed! Sweet multiplier added.");
                      } else {
                        ...
                        setUserNotification(`ðŸš€ Let's go do the "${qst.title}" study milestone!`);
                      }
                    };
              ...
              <div className="space-y-4 pt-2">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    { label: "Apex Scholar", emoji: "âš¡ðŸ¦…", condition: "1200 XP Accumulation", done: (currentUserData?.points || 0) >= 1200, style: "from-cyan-500/20 text-cyan-400" },
                    { label: "Elite Core", emoji: "ðŸ¦¾", condition: "500 XP points achieved", done: (currentUserData?.points || 0) >= 500, style: "from-yellow-500/20 text-yellow-500" },
                    { label: "Sustained Pulse", emoji: "ðŸ”¥", condition: "5 Day streak active", done: (currentUserData?.streak || 0) >= 5, style: "from-emerald-500/20 text-emerald-400" },
                  ...
                  {[
                    ...
                    const handleClaimAchievement = async () => {
                      ...
                      try {
                        ...
                        setUserNotification(`ðŸ† ${ach.title} Claimed! +${ach.xp} XP Multiplier Added.`);
                      } catch (e) {
                                  ...
                                  onClick={async (e) => {
                                    ...
                                    try {
                                      await addDoc(collection(db, 'notifications'), {
                                        ...
                                        type: 'highfive',
                                        title: "âœ‹ High Five Received!",
                                        message: `${currentUserData?.username || 'Somebody'} sent you a High Five for studying hard!`,
                                        timestamp: serverTimestamp(),
                                        read: false
                                      });
                                      setUserNotification(`âœ‹ High-five badge transmitted to ${u.username || u.displayName?.split(' ')[0]}! ðŸ™Œ`);
                                    } catch (err) {
                                  ...
                                  onClick={async () => {
                                    try {
                                      await addDoc(collection(db, 'notifications'), {
                                        ...
                                        fromName: currentUserData?.username || currentUserData?.displayName || 'Scholar',
                                        type: 'streak_request',
                                        title: "ðŸ”¥ Reading Streak Challenge!",
                                        message: `${currentUserData?.username || 'Somebody'} sent you a request to start a 5-day reading streak!`,
                                        timestamp: serverTimestamp(),
                                        read: false
                                      });
                                      setUserNotification(`Sent a 5-day study streak request to ${u.username || u.displayName?.split(' ')[0]}! ðŸ”¥`);
                                    } catch (err) {
