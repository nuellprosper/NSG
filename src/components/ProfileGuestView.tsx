...
export const ProfileGuestView: React.FC<ProfileGuestViewProps> = ({
  ...
  const [otpModalType, setOtpModalType] = useState<'signup' | 'forgot-password'>('signup');
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    // Sign up mode requires email OTP verification first
    if (!isOtpVerifiedForSignup) {
      ...
      try {
        const otpRes = await sendOtpEmail(authEmail, 'signup');
        if (otpRes.success) {
          setOtpExpiresAt(otpRes.expiresAt);
          setOtpModalType('signup');
          setShowOtpModal(true);
          setUserNotification('A 6-digit verification code was sent to your email.');
        } else {
          setUserNotification(`Unable to send OTP: ${otpRes.error || 'Check network connection'}`);
        }
      } catch (err: any) {
        setUserNotification(`Failed to dispatch verification code: ${err.message || err}`);
      } finally {
    ...
    // OTP already verified, execute standard account registration
    handleAuth(e);
  };

  const handleForgotPassword = async () => {
    if (!authEmail || !authEmail.includes('@')) {
      setUserNotification('Please enter your email address in the field above first.');
      return;
    }
    setIsPreparingOtp(true);
    try {
      const otpRes = await sendOtpEmail(authEmail, 'forgot-password');
      if (otpRes.success) {
        setOtpExpiresAt(otpRes.expiresAt);
        setOtpModalType('forgot-password');
        setShowOtpModal(true);
        setUserNotification('Password reset verification code dispatched to your inbox.');
      } else {
        setUserNotification(`Unable to send OTP: ${otpRes.error || 'Check network connection'}`);
      }
    } catch (err: any) {
      setUserNotification(`Failed to send verification code: ${err.message || err}`);
    } finally {
      setIsPreparingOtp(false);
    }
  };
  ...
  const handleOtpVerified = async () => {
    if (otpModalType === 'signup') {
      ...
      setUserNotification('Email verified! Finalizing account creation...');
      const dummyEvent = { preventDefault: () => {} } as React.FormEvent;
      handleAuth(dummyEvent);
    } else if (otpModalType === 'forgot-password') {
      try {
        const { sendPasswordResetEmail: fbSend } = await import('firebase/auth');
        await fbSend(auth, authEmail);
        setUserNotification('Identity confirmed! A secure password reset link has been dispatched to your email.');
      } catch (err: any) {
        setUserNotification(`Failed to dispatch password reset link: ${err.message || err}`);
      }
    }
  };
                  ...
                  <div className="space-y-1.5 text-left">
                    <div className="flex items-center justify-between">
                      ...
                      {authMode === 'login' && (
                        <button 
                          ...
                          className="text-xs font-medium text-orange-400 hover:text-orange-300 transition-colors cursor-pointer disabled:opacity-50"
                        >
                    ...
                    {authMode === 'signup' && (
                      <div className="pt-2">
                        ...
                        {termsError && (
                          <p className="text-[11px] text-red-400 mt-1.5 font-medium flex items-center gap-1 text-left">
