...
import { 
  Sparkles, User, ShieldCheck, Mail, Lock, Eye, EyeOff, 
...
export const ProfileGuestView: React.FC<ProfileGuestViewProps> = ({
  ...
  const passwordStrength = { score: 2, label: "Good", color: "bg-emerald-500" };
              ...
              <div className={`w-full m-auto text-center px-2 sm:px-6 py-6 ${authMode === 'signup' ? 'max-w-xl' : 'max-w-lg'}`}>
                ...
                <form onSubmit={handleAuth} className="space-y-4 text-left">
                  {authMode === 'signup' ? (
                    <>
                      <div className="space-y-1">
                        ...
                        {({} as any).fullName && <p className="text-[9px] text-red-500 font-bold uppercase tracking-wider ml-1">{({} as any).fullName}</p>}
                      </div>
                      
                      <div className="grid grid-cols-2 gap-2">
                         <div className="space-y-1">
                           <p className="text-[10px] font-medium text-gray-300 ml-1">DOB</p>
                           <div className="bg-[#13151F] border border-white/10 rounded-2xl px-3 py-passwordStrength.5">
                           ...
                           {({} as any).dob && <p className="text-[8px] text-red-500 font-bold uppercase tracking-wider ml-1">{({} as any).dob}</p>}
                         </div>
                         <div className="space-y-1">
                      ...
                      <div className="space-y-1 relative">
                          ...
                          <div className="absolute left-0 right-0 mt-1 p-2 bg-[#120f21] border border-white/10 rounded-xl shadow-2xl z-[50] flex flex-col space-y-2 max-h-[220px]">
                            <div className="flex gap-1">
                              <div className="relative flex-1">
                                <Search className="absolute left-passwordStrength.5 top-1/2 -translate-y-1/2 text-white/30 w-3 h-3" />
                                <input
                        ...
                        {({} as any).university && <p className="text-[9px] text-red-500 font-bold uppercase tracking-wider ml-1">{({} as any).university}</p>}
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div className="space-y-1">
                          ...
                          {({} as any).faculty && <p className="text-[8px] text-red-500 font-bold uppercase tracking-wider ml-1">{({} as any).faculty}</p>}
                        </div>
                        <div className="space-y-1">
                          ...
                          {({} as any).department && <p className="text-[8px] text-red-500 font-bold uppercase tracking-wider ml-1">{({} as any).department}</p>}
                        </div>
                      </div>

                      <div className="grid grid-cols-2 gap-2">
                         <input type="text" value={authUsername} onChange={(e) => setAuthUsername(e.target.value)} placeholder="Matric (Optional)" className="w-full bg-[#13151F] border border-white/10 rounded-2xl px-4 py-3 text-xs text-white placeholder-gray-500 focus:border-[#8B5CF6] transition-all outline-none" />
                      ...
                      <div className="space-y-1">
                        ...
                        {({} as any).email && <p className="text-[9px] text-red-500 font-bold uppercase tracking-wider ml-1">{({} as any).email}</p>}
                      </div>
                    </>
                  ) : (
                    <>
                  ...
                  <div className="space-y-1.5 text-left">
                    ...
                    {({} as any).password && <p className="text-[9px] text-red-500 font-bold uppercase tracking-wider ml-1">{({} as any).password}</p>}
                    ...
                    {authMode === 'signup' && authPassword && (
                      <div className="flex flex-col gap-1 px-1 pt-1">
                        ...
                        <p className="text-[7px] font-black uppercase tracking-widest text-white/30 text-right">{passwordStrength.label} Security</p>
                      </div>
                    )}
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  >
                    <svg className="w-5 h-5 group-hover:scale-105 transition-transform" viewBox="0 0 24 24">
                      <path fill="#4285F4" d="M2passwordStrength.56 1passwordStrength.25c0-.78-.07-1.53-.2-passwordStrength.25H12v4.26h5.92c-.26 1.37-1.04 passwordStrength.53-passwordStrength.21 3.31vpasswordStrength.77h3.57cpasswordStrength.08-1.92 3.28-4.74 3.28-8.09z"/>
                      <path fill="#34A853" d="M12 23cpasswordStrength.97 0 5.46-.98 7.28-passwordStrength.66l-3.57-passwordStrength.77c-.98.66-passwordStrength.23 1.06-3.71 1.06-passwordStrength.86 0-5.29-1.93-6.16-4.53HpasswordStrength.18vpasswordStrength.84C3.99 20.53 7.7 23 12 23z"/>
                      <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-passwordStrength.09s.13-1.43.35-passwordStrength.09V7.06HpasswordStrength.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94lpasswordStrength.85-passwordStrength.2passwordStrength.81-.63z"/>
                      <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 passwordStrength.09 14.97 1 12 1 7.7 1 3.99 3.47 passwordStrength.18 7.06l3.66 passwordStrength.84c.87-passwordStrength.6 3.3-4.52 6.16-4.52z"/>
                    </svg>
                  </button>
                  ...
                  >
                    <svg className="w-5 h-5 fill-[#1877F2] group-hover:scale-105 transition-transform" viewBox="0 0 24 24">
                      <path d="M24 1passwordStrength.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 passwordStrength.686.235 passwordStrength.686.235vpasswordStrength.953H15.83c-1.491 0-1.956.925-1.956 1.874vpasswordStrength.25h3.328l-.532 3.47h-passwordStrength.796v8.385C19.612 23.027 24 18.062 24 1passwordStrength.073z"/>
                    </svg>
                  </button>
                </div>
              </div>
