...
export const AILibrary: React.FC<{ 
  ...
  const findLatinMeaning = async () => {
    if (!lawLatinQuery) {
      if (setUserNotification) setUserNotification("Please enter a Latin phrase.");
      return;
    }
          ...
          <AnimatePresence>
            {showVoiceSettings && (
              <motion.div 
                ...
                className="absolute right-0 mt-2 w-64 bg-[#13111C] border border-white/10 rounded-2xl p-4 shadow-2xl z-[60]"
              >
      ...
      <main className="flex-1 overflow-y-auto p-4 space-y-6 pb-24">
        {isAiLoading && (
          <div className="fixed inset-0 z-[100] bg-black/20 backdrop-blur-sm flex items-center justify-center">
            <div className="bg-[#13111C] border border-white/10 p-6 rounded-3xl flex flex-col items-center gap-4 shadow-2xl">
              <RefreshCcw className="animate-spin text-[#DC2626]" size={32} />
              <p className="text-[10px] font-black uppercase tracking-widest text-white/60">Omni Processing...</p>
            </div>
          </div>
        )}
