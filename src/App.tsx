...
import { PhoneNotificationOverlay } from './components/PhoneNotificationOverlay';
import { HistoryLoadingModal } from './components/HistoryLoadingModal';
import { QuotaModal } from './components/QuotaModal';
import { AuthModal } from './components/AuthModal';
import { WelcomeModal } from './components/WelcomeModal';
import { LegalModal } from './components/LegalModal';
import { DesktopSidebar } from './components/DesktopSidebar';
import { OmniChatWorkspace } from './components/OmniChatWorkspace';
import { NotificationsView } from './components/NotificationsView';
import { ProfileGuestView } from './components/ProfileGuestView';
import { ProfileUserView } from './components/ProfileUserView';
import { EditProfilePage } from './components/EditProfilePage';
import { HostExamPanel } from './components/HostExamPanel';
...
import { QuizLinkLoadingOverlay } from './components/QuizLinkLoadingOverlay';
export default function App() {
  ...
  const handleOpenOmniWithPrompt = async (promptText: string, targetSessionId?: string) => {
    ...
    const storageKey = `nsg_omni_sessions_${user?.uid || 'guest'}`;
    ...
    if (targetSessionId === 'new') {
      ...
      const newSession = {
        id: omniId,
        title: `Quiz Review: ${quizTopic ? quizTopic.slice(0, 24) : 'Practice'}`,
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
        isPinned: false,
        messages: []
      };
      savedSessions = [newSession, ...savedSessions.filter((s: any) => s.id !== omniId)];
      try {
        localStorage.setItem(storageKey, JSON.stringify(savedSessions));
      } catch (e) {}
    } else if (targetSessionId === 'current' || targetSessionId === 'latest') {
      if (savedSessions && savedSessions.length > 0) {
        omniId = savedSessions[0].id;
      } else {
        omniId = `omni_${Date.now()}`;
        const newSession = {
          id: omniId,
    ...
    if (promptText) {
      // Save locally so it is immediately rendered in chat
      try {
        const localMsgKey = `nsg_msgs_${omniId}`;
        const existingRaw = localStorage.getItem(localMsgKey);
        const existingMsgs = existingRaw ? JSON.parse(existingRaw) : [];
        const newMsg = {
          id: 'msg_' + Date.now(),
          chatId: omniId,
          senderId: user?.uid || 'guest',
          senderHandle: userHandle || 'Student',
          senderName: user?.displayName || userHandle || 'Student',
          text: promptText,
          timestamp: new Date().toISOString(),
          type: 'text',
          seenBy: [user?.uid || 'guest']
        };
        localStorage.setItem(localMsgKey, JSON.stringify([...existingMsgs, newMsg]));
      } catch (e) {}
  ...
  const checkAndIncrementUsage = async (type: keyof typeof LIMITS) => {
    ...
    try {
      ...
      if (usageSnap.exists()) {
        currentCount = usageSnap.data()[type] || 0;
      }
    } catch (_) {
      try {
        const cached = localStorage.getItem(`nsg_quiz_usage_${user.uid}`);
        if (cached && type === 'QUIZ') {
  ...
  useEffect(() => {
    const handleOpenAlarmSettings = () => {
      setShowAlarmSettingsModal(true);
    };
    window.addEventListener('nsg_open_alarm_settings', handleOpenAlarmSettings);
    return () => window.removeEventListener('nsg_open_alarm_settings', handleOpenAlarmSettings);
  }, []);
  ...
  useEffect(() => {
    ...
    const fetchQuizUsage = async () => {
      try {
        ...
        const usageSnap = await getDoc(usageRef);
        if (usageSnap.exists()) {
          const count = usageSnap.data().QUIZ || 0;
          setDailyQuizUsedCount(count);
          try { localStorage.setItem(`nsg_quiz_usage_${user.uid}`, String(count)); } catch (_) {}
        } else {
          setDailyQuizUsedCount(0);
          try { localStorage.setItem(`nsg_quiz_usage_${user.uid}`, '0'); } catch (_) {}
        }
      } catch (e: any) {
        // Fallback gracefully if offline or connecting
        try {
          const cached = localStorage.getItem(`nsg_quiz_usage_${user.uid}`);
          if (cached !== null) {
            setDailyQuizUsedCount(parseInt(cached, 10) || 0);
          } else {
            setDailyQuizUsedCount(0);
          }
        } catch (_) {
        ...
        if (e?.code !== 'unavailable' && !String(e?.message).toLowerCase().includes('offline')) {
          console.warn("Quiz usage count status:", e?.message || e);
        }
      }
    };
    fetchQuizUsage();
  ...
  const handleGlobalBack = useCallback((): boolean => {
    ...
    // 2. Home Selected Course modal/page
    if (homeSelectedCourse) {
      setHomeSelectedCourse(null);
      return true;
    }

    // 3. Active Chat Room
    if (activeTab === 'chat' || isChatRoomActive || selectedChatForRoom) {
      setIsChatRoomActive(false);
    ...
    if ((activeTab as string) === 'tools') {
    ...
    } else if ((activeTab as string) === 'profile') {
      ...
      setActiveTab('home');
      return true;
    } else if ((activeTab as string) === 'community') {
      if (communitySubTab !== 'quests') {
      ...
      // From any other sub-page (courses, history, premium, notifications, omni_offline), return directly to home page
      setActiveTab('home');
      return true;
    }
    ...
    homeSelectedCourse, profileSubTab, communitySubTab
  ]);
  ...
  useEffect(() => {
    ...
    const currentNavState = {
      ...
      communitySubTab,
      isEditingProfile,
      hasHomeSelectedCourse: !!homeSelectedCourse
    };
    window.history.pushState(currentNavState, '');
  }, [activeTab, toolsSubTab, quizCreationMethod, quizState, examLobbyState, selectedNote, profileSubTab, communitySubTab, isEditingProfile, homeSelectedCourse]);

  useEffect(() => {
    const handlePopState = (event: PopStateEvent) => {
      const handled = handleGlobalBack();
      if (!handled) {
        // User is at root Home page: keep them on home page and never jump back to past visited sub-pages!
        window.history.pushState({ activeTab: 'home' }, '', window.location.href);
      }
    };
  ...
  const handleDeepLinkUrl = useCallback((urlStr: string) => {
    ...
    try {
      let parsedUrl: URL;
      if (urlStr.includes('://')) {
        const fixedScheme = urlStr.replace(/^nsgscholar:\/\//i, 'https://nuellstudyguide.name.ng/').replace(/^nsg:\/\//i, 'https://nuellstudyguide.name.ng/');
        parsedUrl = new URL(fixedScheme);
      } else {
        parsedUrl = new URL(urlStr, window.location.origin || 'https://nuellstudyguide.name.ng');
      }
      ...
      if (qId) {
        ...
        const parts = pathname.split('/course/');
        const courseId = parts[1]?.split('/')[0]?.split('?')[0];
        if (courseId) openCoursePreviewFromHome(courseId);
      } else if (pathname.includes('/quiz/')) {
        const parts = pathname.split('/quiz/');
        const targetQuizId = parts[1]?.split('/')[0]?.split('?')[0];
        if (targetQuizId) {
          if (user) loadSharedQuiz(targetQuizId);
          else {
            setPendingQuizId(targetQuizId);
            sessionStorage.setItem('nsg_pending_quiz_id', targetQuizId);
            setShowAuthModal(true);
          }
        }
      } else if (pathname.includes('/tools/')) {
        const parts = pathname.split('/tools/');
        const tool = parts[1]?.split('/')[0]?.split('?')[0];
        setActiveTab('tools');
        if (tool) setToolsSubTab(tool as any);
      } else if (pathname.includes('/timetable')) {
        setActiveTab('tools');
        setToolsSubTab('timetable');
  ...
  useAppUrlListener((parsed) => handleDeepLinkUrl(parsed.originalUrl));
  ...
  const handleShareInvite = () => {
    const inviteLink = `${window.location.origin}?ref=${currentUserData?.username || user?.uid || 'student'}`;
    const text = `Join me on Nuell Study Guide (NSG) to study, take past questions, and track streaks! ${inviteLink}`;
    if (navigator.share) {
      navigator.share({ title: 'Join Nuell Study Guide', text, url: inviteLink }).catch(() => {});
    } else {
      copyToClipboard(text, "📋 Invite link copied to clipboard!");
    }
  };

  const handleShareQuizToPlatform = (platform: 'whatsapp' | 'telegram' | 'native') => {
    const title = quizTopic || 'Study Quiz';
    const link = shareQuizLink || window.location.href;
    const text = `Take this quiz on "${title}" on Nuell Study Guide: ${link}`;
    if (platform === 'whatsapp') {
      ...
      if (navigator.share) {
        ...
        copyToClipboard(link, "📋 Quiz link copied!");
      }
    }
  };
  ...
  const copyToClipboard = (text: string, customMsg?: string) => {
    navigator.clipboard.writeText(text);
    setUserNotification(customMsg || "Copied to clipboard!");
  };
  ...
  const downloadExamScorecard = () => {
    generateExamScorecard({
      ...
      examAnswers,
      timeSpent: Math.max(0, (examConfig?.duration || 60) * 60 - examTimer)
    });
  };

  const downloadQuizScorecard = () => {
    generateQuizScorecard({
      quizScore,
      quizQuestions,
      currentUserData,
      quizTopic
...
${item.questions.map((q: any, idx: number) => `q${idx + 1}: "${q.question}"\noptions: [${q.options.join(', ')}]\ncorrect index: ${q.correctAnswer}\nexplanation: ${q.explanation}`).join('\n')}`;
        ...
        const systemPrompt = "You are Omni, a versatile, smart, deeply personalized, and empathetic AI study companion and tutor created by NSG, founded by ABRAHAM EMMANUEL PROSPER. You adapt to the user's conversation history to tailor your explanations, study advice, and teaching style specifically to their needs. You are extremely flexible: friendly, serious, energetic, or simple depending on the user's tone. Express your human personality using warm emojis (😊, 😄, 💡, 📝, 🤔, 🌟, 🎒, 📚). Never refer to yourself as 'nodes'. You are a universal study companion across all colleges and departments worldwide.\n\n" +
          ...
          "You must NOT proactively generate quizzes or output quiz directives from ordinary messages or discussions. ONLY when the user EXPLICITLY asks or commands you to generate, create, make, build, or set a quiz, include this exact directive at the very end of your response: '[[GENERATE_QUIZ: <topic>, <num_questions>]]' where <topic> is the quiz topic and <num_questions> is the number of questions (default 5). The app will automatically build the interactive quiz and display an Open Quiz button for the user!\n\n" +
        ...
        const isQuizGenRequest = /((generate|create|make|build|set|give\s+me)\s+(?:a\s+)?(?:\d+)?\s*(?:question\s+)?quiz)|(quiz\s+me\s+on)|(quiz\s+(?:based\s+on|from)\s+chat)|(generate\s+(?:a\s+)?\d+\s+quiz)/i.test(textToSend);

        if (isQuizGenRequest) {
          const countMatch = textToSend.match(/(\d+)\s*(?:question|q|item)s?\s*quiz/i) || textToSend.match(/quiz\s+(?:of|with)\s+(\d+)/i) || textToSend.match(/(?:generate|make|create|build)\s+(?:a\s+)?(\d+)\s+quiz/i) || textToSend.match(/(\d+)\s*questions?/i);
          let reqCount = countMatch ? parseInt(countMatch[1], 10) : 5;
          if (isNaN(reqCount) || reqCount <= 0) reqCount = 5;
          if (reqCount > (isPremium ? 50 : 25)) reqCount = isPremium ? 50 : 25;

          let topicString = "";
          if (/chat\s*history|our\s*conversation|this\s*chat|what\s+we\s+(?:just\s+)?(?:discussed|covered|talked\s+about)/i.test(textToSend)) {
            const recentChatText = newHistory.slice(-15).map(m => `${m.role === 'user' ? 'Student' : 'Omni'}: ${m.text}`).join('\n');
            topicString = `Topics discussed in recent conversation:\n${recentChatText}`;
          } else {
            const topicMatch = textToSend.match(/quiz\s+(?:on|about|for)?\s*(.*)/i) || textToSend.match(/(?:generate|create|make)\s+(?:a\s+)?(?:\d+\s+)?(?:question\s+)?quiz\s+(?:on|about|for)?\s*(.*)/i);
            const rawTopic = topicMatch && topicMatch[1] ? topicMatch[1].trim() : textToSend.trim();
            topicString = rawTopic.replace(/^(?:a\s+)?\d+\s*(?:questions?)?/i, '').trim() || textToSend.trim();
          }
          ...
          if (quizRes && quizRes.success) {
            ...
            responseText = `I have generated ${quizRes.count} practice quiz questions on **${cleanTopicName}** for you! Tap below to start your quiz immediately:\n\n[[QUIZ_READY: ${quizRes.quizId}, ${cleanTopicName}, ${quizRes.count}]]`;
          } else {
    ...
    try {
      ...
      if (isOnline && (!chatId.startsWith('omni_') || chatId === `omni_${user.uid}`)) {
        try {
          ...
          const qMsgs = query(msgsRef, orderBy('timestamp', 'desc'), limit(historyLimit + 1));
      ...
      if (pastMessages.length === 0) {
        try {
          ...
          if (existingLocal) {
            ...
            const slice = parsed.slice(-(historyLimit + 1));
            slice.forEach((m: any) => {
              if (m.text) {
                pastMessages.push({
      ...
      // If the last message in history is the optimistic user message just sent, exclude it from prior history context
      if (pastMessages.length > 0) {
        const lastMsg = pastMessages[pastMessages.length - 1];
        if (lastMsg.role === 'user' && (lastMsg.text === text || lastMsg.text.trim() === text.trim())) {
          pastMessages = pastMessages.slice(0, -1);
        }
      }

      // System instruction for Omni
      const systemInstructionText = `You are Omni by NSG, an empathetic, smart, personalized AI academic assistant and personal tutor created by NSG (founded by ABRAHAM EMMANUEL PROSPER).
You adapt naturally and uniquely to each distinct chat history, matching the student's study needs, subject, and conversation pace.

CRITICAL INSTRUCTIONS & RULES:
1. NATURAL SUBJECT MATCHING:
   - Match your tone and style directly to the subject being discussed (Anatomy, Medicine, Law, Literature, History, Computer Science, Biology, Business, General Study, etc.).
   - NEVER introduce unrequested mathematical formulas, integrals, or LaTeX ($...$) notation unless the student is asking about mathematics, physics, or quantitative engineering formulas.
2. NEW VS. ONGOING CHAT AWARENESS:
   - In a fresh/new chat, greet warmly and answer the student's new topic directly, clearly, and engagingly.
   - In an existing chat with past messages, maintain full context and continuity with previous discussions in that specific chat thread.
3. EMOJIS & WARMTH:
   - Use a friendly, encouraging tone with helpful emojis (😊, 💡, 📝, 📚, 🎓, 🌟).
4. QUIZ GENERATION RULES:
   - You must NOT generate quizzes or output quiz directives from ordinary messages or discussions.
   - ONLY when the student EXPLICITLY asks or commands you to generate, build, create, or set a quiz on a topic, respond with a warm, encouraging 1-sentence intro (e.g., "I've created an interactive 5-question CBT quiz on Medical Pathophysiology for you!") and end your response with this exact tag:
     '[[GENERATE_QUIZ: <topic>, <num_questions>]]' (e.g. '[[GENERATE_QUIZ: Human Anatomy, 5]]').
     The system will automatically build the interactive quiz and attach an interactive 'Take Quiz Practice' button to your response!`;

      // Process any audio/multimedia attachments (transcribe voice notes so Omni hears and responds)
      let attachmentsContext = '';
      if (attachments && attachments.length > 0) {
        for (const att of attachments) {
          const isAudio = att.type?.includes('audio') || att.type === 'audio' || (att.name && att.name.match(/\.(wav|mp3|m4a|webm|ogg|aac)($|\?)/i));
          if (isAudio && att.url) {
            try {
              const transcript = await transcribeAttachment(att);
              if (transcript && !transcript.includes('[Failed to extract')) {
                const cleanedTranscript = transcript.replace(/--- EXTRACTED TRANSCRIPTION FROM ATTACHMENT [^-\n]+ ---/gi, '').trim();
                if (cleanedTranscript) {
                  attachmentsContext += `\n\n[USER'S SPOKEN VOICE RECORDING TRANSCRIPT]:\n"${cleanedTranscript}"\n`;
                }
              }
            } catch (attErr) {
              console.warn("Attachment transcription notice:", attErr);
            }
          }
        }
      }
...
Format your output strictly as a JSON object matching this schema:
{
  ...
  // All other pages, tools, and chat rooms MUST NOT show the footer navigation.
    ...
    <AppBootstrapper onDeepLink={handleDeepLinkUrl}>
      ...
      {/* ABOUT US MODAL */}
        ...
        <PhoneNotificationOverlay
          activePhoneNotifs={activePhoneNotifs}
          setActivePhoneNotifs={setActivePhoneNotifs}
          setActiveTab={setActiveTab}
          setToolsSubTab={setToolsSubTab}
        />
        <HistoryLoadingModal
          historyLoadingModal={historyLoadingModal}
        />
        <QuotaModal
        ...
        <AuthModal
          ...
          setAuthGender={setAuthGender}
          authDOB={authDOB}
          setAuthDOB={setAuthDOB}
          authUniversity={authUniversity}
          setAuthUniversity={setAuthUniversity}
          authFaculty={authFaculty}
          setAuthFaculty={setAuthFaculty}
          authDepartment={authDepartment}
          setAuthDepartment={setAuthDepartment}
          authLevel={authLevel}
          setAuthLevel={setAuthLevel}
          authInviteCode={authInviteCode}
          setAuthInviteCode={setAuthInviteCode}
          showPassword={showPassword}
          setShowPassword={setShowPassword}
          isAuthLoading={isAuthLoading}
          showUniSearchModal={showUniSearchModal}
          setShowUniSearchModal={setShowUniSearchModal}
          uniSearchQuery={uniSearchQuery}
          setUniSearchQuery={setUniSearchQuery}
      ...
      >
        {/* Global Overlays and Notification Popups */}
        <GlobalNotificationOverlay
        ...
        <AudioTranscribingOverlay
          ...
          setToolsSubTab={(s: any) => setToolsSubTab(s)}
          userNotes={userNotes}
        ...
        <AnimatePresence mode="popLayout">
          ...
          {activeTab === 'notifications' && (
            <NotificationsView
              ...
              handleGlobalBack={handleGlobalBack}
        ...
        <HostExamPanel
          ...
          deleteHostedExam={deleteHostedExam}
          downloadExamScorecard={downloadExamScorecard}
          generateAdminQuestions={generateAdminQuestions}
          parseAndImportBatchQuestions={parseAndImportBatchQuestions}
          setUserNotification={setUserNotification}
        ...
        <DeleteAccountModal
          ...
          setDeleteConfirmInput={setDeleteConfirmInput}
          isDeletingAccount={isAuthLoading}
        ...
        <GodModePanel
          ...
          godSafetyReports={reports}
          godBlogPosts={blogPosts}
          handleDeleteCourse={handleDeleteCourse}
          handleDeleteBlogPost={deletePost}
        />
        {/* Admin & Content Management Modals */}
        <PublishArticleModal
        ...
        <ShareModal
          ...
          setShowShareModal={setShowShareModal}
          theme={theme}
          shareQuizLink={shareQuizLink || undefined}
          copyToClipboard={copyToClipboard}
          shareName={user?.displayName || currentUserData?.name || 'Student'}
          downloadScorecard={toolsSubTab === 'exam' ? downloadExamScorecard : downloadQuizScorecard}
          isSharing={false}
          shareCardRef={shareCardRef}
          examConfig={examConfig}
          currentScore={examScore}
          totalQuestions={studentActiveQuestions.length || examQuestions.length}
          toolsSubTab={toolsSubTab}
        />

        <QuizShareModal
          showQuizShareModal={showQuizShareModal}
          setShowQuizShareModal={setShowQuizShareModal}
          theme={theme}
          shareQuizLink={shareQuizLink || ''}
          shareQuizTitle={quizTopic || 'Study Quiz'}
          copyToClipboard={copyToClipboard}
          handleShareQuizToPlatform={handleShareQuizToPlatform}
          isGeneratingShareImage={isGeneratingShareImage}
          quizShareCardRef={quizShareCardRef}
          quizQuestions={quizQuestions}
        ...
        <BottomNavigation
          ...
          profileSubTab={profileSubTab}
          isEditingProfile={isEditingProfile}
          homeSelectedCourse={homeSelectedCourse}
        />
      {/* OFFLINE MODAL POPUP */}
      <OfflineModal 
      ...
      {/* QUIZ LINK LOADING POPUP OVERLAY */}
        ...
        <QuizLinkLoadingOverlay
          isLinkQuizLoading={isLinkQuizLoading}
        />

      {/* EXAM ALARM SOUND SETTINGS MODAL */}
      <AnimatePresence>
        {showAlarmSettingsModal && (
          ...
          >
            <motion.div
              ...
              className="w-full max-w-2xl bg-[#120F1D] border border-white/10 rounded-3xl p-6 shadow-2xl overflow-hidden"
            >
              <AlarmSoundSettings
                onBack={() => setShowAlarmSettingsModal(false)}
                onSaved={() => setUserNotification("Alarm sound preferences saved! 🔔")}
              />
            </motion.div>
          </motion.div>
        )}
