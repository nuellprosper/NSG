...
export default function App() {
    ...
    if (leagueName === "Bronze League") {
      return [
        ...
        },
        {
          ...
          desc: "Invite one friend to the node (Earn final 5 XP to clear Bronze!)",
  ...
  const generatePodcastDiscussion = async (sourceContent: string) => {
    ...
    try {
      ...
      - Actively discuss, ask, and answer questions about the images, the texts, and ALL files in this note. Highlight specific aspects seen or described in those attachments!
...
Respond professionally, concisely, and use LaTeX for math.` }];
  ...
  const generateQuiz = async () => {
    ...
    try {
      ...
      const prompt = `
        Generate a ${quizQuestionCount}-question multiple choice quiz on the user's specific academic topic.
        The questions must strictly cover and test the user's requested topic/context: "${quizTopic}".
        ${quizImages.length > 0 ? "Analyze the attached image(s) to generate relevant academic questions covered on them." : ""}
